...
from libs.shape import Field, Shape, DEFAULT_LINE_COLOR, DEFAULT_FILL_COLOR
...
from libs.form_io import FormReader
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def set_format(self, save_format):
        ...
        elif save_format == FORMAT_FORM:
            self.actions.save_format.setText(FORMAT_FORM)
            self.actions.save_format.setIcon(newIcon("format_form"))
            self.usingPascalVocFormat = False
            self.usingYoloFormat = False
            self.usingFormFormat = True
    ...
    def change_format(self):
        ...
        elif self.usingYoloFormat: self.set_format(FORMAT_FORM)
        elif self.usingFormFormat: self.set_format(FORMAT_PASCALVOC)
    ...
    def saveLabels(self, annotationFilePath):
        ...
        fields = [format_field(field) for field in self.fields]
        ...
        try:
            ...
            if self.usingFormFormat is True:
                if annotationFilePath[-5:].lower() != ".json":
                    annotationFilePath += JSON_EXT
                self.labelFile.saveFormFormat(annotationFilePath, fields,
    ...
    def newShape(self):
        ...
        if text is not None:
            ...
            if self.isField and not self.isKey:
                self.fields += [
                    Field(self.canvas.shapes[-2], self.canvas.shapes[-1])
                ]
                self.isField = False
                self.isKey = False
    ...
    def loadFile(self, filePath=None):
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            if self.defaultSaveDir is not None:
                ...
                jsonPath = os.path.join(self.defaultSaveDir,
                                        basename + JSON_EXT)
                ...
                if os.path.isfile(jsonPath):
                    self.loadFormJSONByFilename(jsonPath)
    ...
    def loadFormJSONByFilename(self, jsonPath):
        if self.filePath is None:
            return
        if os.path.isfile(jsonPath) is False:
            return

        self.set_format(FORMAT_FORM)

        tFormParseReader = FormReader(jsonPath)
        shapes = tFormParseReader.getShapes()
        self.loadLabels(shapes)
        self.canvas.verified = tFormParseReader.verified
